...
class Blackjack:
    ...
    def __init__(self, wallet):
        ...
        self.game_number = self.num_games
        self.log = ""
    def play_round(self, num_rounds, stand_threshold):
        ...
        new_bet = Blackjack.bet
        for i in range(num_rounds):
            player = PlayerHand()
            dealer = DealerHand()
            if len(self.deck.cards) >= 4:
                if self.wallet >= new_bet:
                    self.log += "Round {} of Blackjack!\n".format(self.n) + "wallet: {}\n".format(self.wallet) + "bet: {}\n".format(new_bet)
                    times1 = randint(6)
                    times2 = randint(6)
                    self.deck.shuffle(mongean = times1, modified_overhand = times2)
                    ...
                    play_score = Blackjack.calculate_score(player)
                    deal_score = Blackjack.calculate_score(dealer)
                    result = self.determine_winner(play_score, deal_score)
                    ...
                    self.log += "Wallet amount $" + str(self.wallet) + " is less than bet amount $" + str(new_bet) + "."
                    break
            else:
    ...
    def calculate_score(hand):
        ...
        dicts = {"J":10,"Q":10,"K":10}
        a = hand.get_cards()
        a = list(map(lambda x: x.rank, a))
        no_a = [each for each in a if each != "A"]
        no_a = map(lambda x: dicts[x] if x in dicts else x,no_a)
        sum_wo_a = sum(no_a)
        num_a = len([each for each in a if each == "A"])
        if num_a + sum_wo_a > 21:
            return num_a + sum_wo_a
        elif num_a * 11 + sum_wo_a <= 21:
            return num_a * 11 + sum_wo_a
        else:
            all_scores = [sum_wo_a + 11 * i + (num_a - i) for i in range(num_a)]
            all_scores = filter(lambda x: x <= 21 ,all_scores)
    ...
    def hit_or_stand(self, hand, stand_threshold):
        ...
        while Blackjack.calculate_score(hand) < stand_threshold and len(self.deck.cards) > 0:
            to_be_dealt = self.deck.get_cards()[0]
            to_be_dealt.set_visible(True)
            self.deck.deal_hand(hand)
            if isinstance(hand, DealerHand):
                self.log += 'Dealer pulled a ' + repr(to_be_dealt) + '\n'
            else:
                self.log += 'Player pulled a ' + repr(to_be_dealt) + '\n'
    ...
    def add_to_file(self, player_hand, dealer_hand, result):
        ...
        with open ("./game_summaries/game_summary" + str(self.game_number) + ".txt", "a", encoding = "utf-8" ) as f:
            f.write("ROUND " +  str(self.n - 1) + ":\n")
            f.write("Player Hand:\n")
            f.write(str(player_hand) + "\n")
            f.write("Dealer Hand:\n")
            f.write(str(dealer_hand) + "\n")
            if result == 1:
                f.write("Winner of ROUND " + str(self.n - 1) + ": Player\n\n")
            elif result == -1:
                f.write("Winner of ROUND " + str(self.n - 1) + ": Dealer\n\n")
            else:
                f.write("Winner of ROUND " + str(self.n - 1) + ": Tied\n\n")
